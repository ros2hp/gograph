...
func New(name string) *Event {
	...
	e := &Event{eid: eid, event: name, start: et, TxHandle: tx.NewTx("EV-" + name).DB("mysql-ros2hp/gograph")}
...
func NewContext(ctx context.Context, name string) *Event {
	...
	// create event and assign transaction handle
	e := &Event{eid: eid, event: name, start: et, TxHandle: tx.NewTxContext(ctx, "EV-"+name).DB("mysql-ros2hp/gograph")}
