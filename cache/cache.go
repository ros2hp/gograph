...
// NewMutation is a helper function to create ros2hp/gograph related mutations - for tables registered in tbl package.
...
func (pn *NodeCache) PropagationTarget(txh *tx.Handle, cpy *blk.ChPayload, sortK string, pUID, cUID uuid.UID) {
	...
	if oBlocks <= param.MaxOvFlBlocks {
		...
		// only interested in last entry in Nd, Id UID-pred arrays, as that is the one ros2hp/gograph
