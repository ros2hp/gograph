...
import (
	...
	"github.com/ros2hp/gograph/attach-merge/ds"
	"github.com/ros2hp/gograph/attach-merge/execute"
...
func fetchParentNode(qtx *tx.QHandle) []pNodeBid {
	...
	for _, lc := range []int{1, 2} {
		...
		//qtx := tx.NewQuery2(ctx, "EdgeParent", tblEdge).DB("mysql-ros2hp/gograph").Prepare()
